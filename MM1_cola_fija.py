...
# PARAMETROS (modificar aca)

mu = 4                    # tasa de servicio
capacidad_cola = 5        # capacidad maxima de la cola (finito)
tiempo_total = 100        # tiempo total de simulacion
dt = 0.01                 # paso de tiempo
repeticiones = 10         # 10 corridas obligatorias
porcentajes = [0.25, 0.50, 0.75, 1.00, 1.25]


# SIMULACION M/M/1

def simular(lambda_arribo):
    en_cola = 0
    ...
    tiempo_en_n_clientes = {i: 0 for i in range(capacidad_cola + 1)}
    ...
    while tiempo_actual < tiempo_total:
        ...
        if random.random() < p_llegada:
            ...
            if servidor_ocupado and en_cola >= capacidad_cola:
...
# BUCLE PRINCIPAL (EXPERIMENTACION)

print("=" * 85)
print("SIMULACION M/M/1")
print(f"Mu = {mu} | Capacidad max cola = {capacidad_cola} | Tiempo max = {tiempo_total}")
print("=" * 85)
...
for porcentaje in porcentajes:
    ...
    acum_prob_n = {i: 0.0 for i in range(capacidad_cola + 1)}

    for rep in range(repeticiones):
        resultado = simular(lambda_arribo)
    ...
    string_probs = ", ".join([f"P({n})={acum_prob_n[n]/repeticiones:.2%}" for n in range(min(3, capacidad_cola+1))])
